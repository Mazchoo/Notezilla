...
- [x] Test Round-trip conversion (Markdown -> DB Mock -> Markdown) preserves all data without loss.
- [x] Validation rejects notes missing required fields (`path`, `content`) with clear error messages.
- [x] The `tags` field accepts a list of strings; the `path` field represents an ordered file tree location.
- [x] At least 3 example notes demonstrate the schema in both JSON and Markdown formats.
